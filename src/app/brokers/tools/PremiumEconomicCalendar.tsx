...
const PremiumEconomicCalendar = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (data.success && data.events && Array.isArray(data.events) && data.events.length > 0) {
        ...
        console.log('Sample event times:', data.events.slice(0, 3).map((e: any) => ({ event: e.event, time: e.time, timestamp: new Date(e.timestamp).toLocaleString() })));
      } else {
  ...
  // Check if an event is happening now or soon
  const isEventLive = (eventTime: number) => {
    const now = new Date().getTime();
    const eventTimeMs = eventTime;
    const timeDiff = Math.abs(eventTimeMs - now);
    // Event is considered live if within 30 minutes
    return timeDiff <= 30 * 60 * 1000 && eventTimeMs > now;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-zinc-900/50 rounded-xl p-4 border border-zinc-800">
        ...
        {!getEventsCountForDate(selectedDate) && events.length > 0 && (
          <div className="mt-3 text-center text-sm text-zinc-500">
            No economic events scheduled for this date in {deviceTimezone} timezone
          </div>
        )}
      </div>
            ...
            <tbody className="divide-y divide-zinc-800">
              {sortedEvents.length > 0 ? (
                sortedEvents.map((event) => {
                  const isLive = isEventLive(event.timestamp);
                  return (
                    <tr key={event.id} className={`hover:bg-white/5 transition-colors ${isLive ? 'bg-green-500/5' : ''}`}>
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-2">
                          {isLive && <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>}
                          <Clock size={14} className="text-zinc-500" />
                          <span className={`font-mono ${isLive ? 'text-green-400 font-bold' : 'text-white'}`}>
                            {event.time}
                          </span>
