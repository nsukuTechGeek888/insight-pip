// app/api/brokers/slug/[slug]/route.ts - COMPLETE WITH REGION CHECK
...
export async function GET(
  ...
  try {
    ...
    console.log('📡 API Request for broker slug:', slug);
    ...
    if (!broker) {
      console.log('🔍 Broker not found by slug, trying by name:', slug.replace(/-/g, ' '));
    ...
    if (!broker) {
      console.log('❌ Broker not found with slug:', slug);
    ...
    const brokerSlug = broker.slug || broker.name.toLowerCase().replace(/\s+/g, '-');
